...
The workflow helper launches its fixture with `process.execPath`. macOS DMG and ZIP inspection locates exactly one non-symlink `.app` bundle within a bounded depth and applies required-resource checks relative to that bundle. Linux hosted UI smoke runs under a private Xvfb display because the runner has no X server. Electron Builder keeps the `runAsNode` fuse explicit for the packaged worker. Unix Runtime verification uses a temporary external ESM probe to import the exact asar entry; Windows starts the normal packaged Main with an explicit test-only probe flag that imports the same entry from inside the app. Both paths pipe output and record the exit code, signal, forced-failure state, and ready marker alongside the bounded diagnostic; timeout cleanup terminates only that child tree and retries removal of its owned directory.
