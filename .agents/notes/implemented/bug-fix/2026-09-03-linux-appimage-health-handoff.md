...
The workspace declares x64 and arm64 optional dependency resolution for the current operating system. The macOS builder retains identical target-native Mach-O package files while still lipo-merging files that differ by architecture. The manylinux job mounts `RUNNER_TEMP` read-only at the same path used by the generated Makefile. The CLI release path uses `pnpm deploy --legacy` with a hoisted node_modules tree, materializes the Linux node-pty package when legacy deploy omits its top-level link, and removes devDependencies before packing; workspace packages therefore do not retain injected links to the source tree. The hosted job restores the full `prod=false` workspace after packed-CLI smoke before later release commands. The isolated Deb fixture uses `--force-depends` only inside its owned dpkg root because the root does not contain the runner's system packages; it still requires configured package state, expected files, launch, and unchanged host state.

The workflow helper launches its fixture with `process.execPath`. macOS DMG and ZIP inspection locates exactly one non-symlink `.app` bundle within a bounded depth and applies required-resource checks relative to that bundle. Linux hosted UI smoke runs under a private Xvfb display because the runner has no X server. Electron Builder keeps the `runAsNode` fuse explicit for the packaged worker. The local Runtime package exports its test-only `./bin` entry explicitly. Unix Runtime verification uses a temporary external ESM probe to import the exact asar entry; Windows starts the normal packaged Main with an explicit test-only probe flag that imports the same entry through that package export. The Windows probe sets the Runtime's test-only module-load mode, so it verifies the packaged entry and static dependency graph without entering the long-lived stdin lease. Windows worker-directory validation normalizes extended-path prefixes and canonical separators before comparing paths, while still rejecting links and escapes from the private update root. Post-packed release checks invoke pinned Node, Vitest, or Playwright entries directly so pnpm dependency-status checks cannot mutate the workspace. Standalone archive verification compares real paths after extraction, so archive symlinks do not produce false host-runtime failures; launcher diagnostics retain bounded exit, stdout, and stderr fields. Both Desktop Runtime paths pipe output and record the exit code, signal, forced-failure state, and ready marker alongside the bounded diagnostic; timeout cleanup terminates only that child tree and retries removal of its owned directory. Failed installed-Desktop smoke writes bounded Playwright error-context files to a non-release diagnostic artifact.
