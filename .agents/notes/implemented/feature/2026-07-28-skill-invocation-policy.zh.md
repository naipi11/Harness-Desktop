...
面向模型的 `dsh-tool-skill` 目录和 loader 执行 `isModelInvocable`。`skill.list` 会公开每个允许用户调用的条目及其 `modelInvocable` 标志；通过 `session.prompt` 提交、以 `/name` 开头的精确输入会优先交给已注册 command，随后只有在精确 Agent 的 scope 链可达 `dsh-tool-skill` pre-step 消费方时，才准入目录命中。该消费方仅在两个监听器均存活后挂接自身受 effect 作用域约束的注册，并在注入前重新检查已加载定义。缺少消费方或完整目录未命中时返回 `unknown-command`，且不产生模型轮次；消费方存在时，发现不完整会返回 `internal`，避免把暂时的不确定状态误判或准入。
...
**把完整的用户可调用目录视为斜杠加载可用的证明。** 不予采纳，因为提供方与 pre-step 消费方可以独立组合。准入必须确认精确 Agent 可达一项受 effect 作用域约束的消费方注册，因此相邻 preset 或仅含提供方的组合无法在缺少正文加载监听器时把原始斜杠文本送入模型。
...
无密钥 ACP（Agent Client Protocol）快照固定了模型目录的变更：其中包含仅允许模型调用的 skill，并排除仅允许用户调用的 skill。组装后的无密钥 Runtime 快照挂载真实 pre-step 消费方并加载仅允许用户调用的 skill；随后，它会启动一个目录同样完整但不含消费方的组合，并证明结果为 `unknown-command`、不注入正文且不产生 Agent 轮次。同一快照还保留了已识别 command 的零轮次行为与真正未知名称的拒绝。注册表、本地解析器、模型工具、tool-skill 和 API 代理测试覆盖策略默认值、支持的布尔写法、格式错误的值、旧键拒绝、精确加载时的策略执行、scope 所有权和 dispose。
