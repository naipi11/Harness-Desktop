...
`maxSourceBytes` 是正数的逐文件上限，默认为 1 MiB；`maxTotalSourceBytes` 是正数的批次总上限，默认为 4 MiB。loader 会在读取前检查报告的大小。提供方 target 使用包含上限的原始字节 `readBytes()` API，在逐文件上限与剩余总预算中取较小值，然后严格按 UTF-8 解码；主机文件使用有界流读取。因此缺失／陈旧的元数据不会造成提供方超额交付或无界分配；基线或动态批次达到总上限后会停止纳入后续文件。过大或无效 UTF-8 的胜出候选项会被视为不可用，而不是改为同目录中的下一个名称。插件刻意不保留进程级缓存，也绝不保留指令正文。它只为每个有效作用域保存 `{ path, version, digest }`，并将这些状态放在 `WeakMap<Session, Map<scope, state>>` 中：提供方 `FsVersion` 与有效提示词状态同时匹配时跳过读取；版本变化则触发有界读取和 SHA-1 确认。SHA-1 仍是持久化在可见结构化元数据中的跨提供方内容标识；提供方版本只作为内存中的失效快速路径。动态缓存转换发生在串行投影组合唯一一条目标 inbox 上下文时；下一次 pre-step 会等待该投影完成，再决定哪些内容进入请求。
