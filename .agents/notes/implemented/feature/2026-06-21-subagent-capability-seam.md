...
| Package | Role |
|---|---|
| `@harness-desktop/dsh-subagent` | interface: `SubagentRuntime` (`ctx.subagents`), `SubagentProvider`, `SubagentRun`, the request/result/capability vocabulary, the `subagent/*` events |
| `@harness-desktop/dsh-subagent-spawn-in-process` | implementation: a fresh in-process child via `ctx.agents.create` |
| `@harness-desktop/dsh-subagent-fork-in-process` | implementation: an in-process child seeded with a snapshot of the parent's log |
| `@harness-desktop/dsh-subagent-acp` | implementation: an ACP client driving a configured child process |
| `@harness-desktop/dsh-subagent-codex` | implementation: a one-shot official Codex app-server process |
| `@harness-desktop/dsh-subagent-claude-code` | implementation: a one-shot official Claude Code process through the Agent SDK |
| `@harness-desktop/dsh-tool-subagent` | consumer: the model-facing `subagent` tool over `ctx.subagents` |
