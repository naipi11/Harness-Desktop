...
1. `@harness-desktop/dsh-web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@harness-desktop/dsh-web-search-exa`, `@harness-desktop/dsh-web-search-perplexity`, `@harness-desktop/dsh-web-search-deepseek`, and `@harness-desktop/dsh-web-fetch-http`.
3. `@harness-desktop/dsh-tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@harness-desktop/dsh-tool-web  --depends on-->  @harness-desktop/dsh-web  <--depends on--  @harness-desktop/dsh-web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @harness-desktop/dsh-web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @harness-desktop/dsh-web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @harness-desktop/dsh-web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@harness-desktop/dsh-web-search-exa"] -->|registerSearchProvider| web["@harness-desktop/dsh-web / ctx.web"]
  perplexity["@harness-desktop/dsh-web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@harness-desktop/dsh-web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@harness-desktop/dsh-web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@harness-desktop/dsh-tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@harness-desktop/dsh-web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@harness-desktop/dsh-tool-web` depends on `@harness-desktop/dsh-web`, `@harness-desktop/dsh-tools`, `@harness-desktop/dsh-system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@harness-desktop/dsh-web'
...
```yaml
- id: web
  name: '@harness-desktop/dsh-web'
  config:
    searchProvider: exa
    fetchProvider: http

- id: web-search-exa
  name: '@harness-desktop/dsh-web-search-exa'

- id: web-search-perplexity
  name: '@harness-desktop/dsh-web-search-perplexity'

- id: web-search-deepseek
  name: '@harness-desktop/dsh-web-search-deepseek'

- id: web-fetch-http
  name: '@harness-desktop/dsh-web-fetch-http'
