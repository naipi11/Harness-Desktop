...
Each startup mints a fresh Foundation `DashboardNavigation`. A handoff minted by `RuntimeClient` retains that native client as the Dashboard cookie's control owner, so Main and its browser attachment observe and stop the same work; an ownerless browser launch retains the cookie-derived owner. The Main-only transport validates an exact `http://127.0.0.1:<port>` origin and an unexpired opaque handoff, writes the handoff only into one hidden field in an owner-only local document, and loads only its secret-free file path. The top-level form POST carries the handoff from an opaque file origin. The Runtime consumes it once, returns a clean 303 with an HttpOnly `SameSite=Strict` session cookie and no CORS permission, and reflects no request body into diagnostics.
...
Main observes the Desktop Runtime client's exact `ActiveWorkStatus` before a native window close. Active work presents only `minimize-to-tray`, `safely-stop-own-ui-work`, and `cancel`: minimize lazily creates the platform tray and hides the existing window without releasing its owner; safe stop awaits the exact owner-scoped `OwnUiWorkStopResult` before retiring that window's attachment and client; cancel changes nothing. A claimed-work stop waits for Agent quiescence and ends its Runtime lease even without a separate `turn/end` callback. A failed safe stop remains open and presents only Foundation's redacted diagnostic. The tray exposes Restore and Quit; Restore shows and focuses the same Dashboard window, while Quit repeats the same close decision and exits the application only after a closed result. Idle close and application shutdown retain attachment-before-client release and never stop the Runtime, release a background lease, or cancel foreign work.
...
**Put active-work close state in the Dashboard renderer.** A renderer-owned close prompt would duplicate native window and tray lifecycle, and it could disappear during renderer loss. Main consumes Foundation's owner-scoped operations directly; the Dashboard remains the workbench owner.
...
Desktop startup depends on a private temporary file and an extra browser exchange, and Main must retain cleanup, expiry, navigation, response ownership, recovery, close-decision, and tray state until they settle. Real Chromium and Electron coverage pins the opaque-origin POST, 303 and cookie sequence, clean URL, absent CORS permission, exact WebSocket CSP, denied foreign navigation, lack of URL, referrer, storage, header, console, or DOM leakage, and preservation of the existing Dashboard workbench across hide and restore. The Electron journey also pins initial Runtime start failure, a failed user retry with diagnostic replacement, a later successful retry, paced streaming tool output, and a real pending approval resolved through the Dashboard. Unit coverage pins concurrent startup, explicit retry, closed-window races, per-navigation marker checks, pending-probe aborts, every close decision, tray actions, and attachment-before-client shutdown.
