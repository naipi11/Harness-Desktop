...
Main observes the Desktop Runtime client's exact `ActiveWorkStatus` before a native window close. Active work presents only `minimize-to-tray`, `safely-stop-own-ui-work`, and `cancel`: minimize lazily creates the platform tray and hides the existing window without releasing its owner; safe stop awaits the exact owner-scoped `OwnUiWorkStopResult` before retiring that window's attachment and client; cancel changes nothing. A failed safe stop remains open and presents only Foundation's redacted diagnostic. The tray exposes Restore and Quit; Restore shows and focuses the same Dashboard window, while Quit repeats the same close decision. Idle close and application shutdown retain attachment-before-client release and never stop the Runtime, release a background lease, or cancel foreign work.
