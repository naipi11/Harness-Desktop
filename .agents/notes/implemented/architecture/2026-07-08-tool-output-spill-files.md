...
| Package | Role |
|---|---|
| `@harness-desktop/dsh-spill` | Interface: `ctx.spillStore`, vocabulary types, no storage implementation. |
| `@harness-desktop/dsh-spill-local` | Local backend: private, session-scoped file storage on the host filesystem. |
| `@harness-desktop/dsh-spill-policy` | Tool-result policy plugin: wraps final text results after dispatch and replaces oversized results with a retained preview plus a spill locator. |
...
- `@harness-desktop/dsh-output-retention` owns preview mechanics (`TextRetainer`, `ItemRetainer`, and omitted metadata).
- `@harness-desktop/dsh-spill` owns saving final text and returning a locator plus retrieval hint.
- `@harness-desktop/dsh-spill-policy` applies the default final-result policy in the tool pipeline, composing the two.
