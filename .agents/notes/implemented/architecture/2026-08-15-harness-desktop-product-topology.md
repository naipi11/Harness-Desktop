...
The foundation boots the same complete base-and-Web composition from its declared built binary and direct source entry. An Electron Main caller launches that Runtime child through its own executable with child-only `ELECTRON_RUN_AS_NODE=1`; Node callers retain their environment, and the setting does not enter Renderer. Terminal, Web, and Desktop presentation work consumes this public API as separate product layers; foundation acceptance alone does not represent those presentation clients or cross-client product acceptance as shipped.
