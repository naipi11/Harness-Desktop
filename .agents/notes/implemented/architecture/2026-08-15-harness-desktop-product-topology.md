...
The CLI Web mode attaches through that public API, opens the Dashboard with the body-only handoff, and uses the stable `web` lease instead of a per-command Web child. The Runtime API exposes no exchange-settlement signal to the launcher, so the launcher retains its owner-only bootstrap through dispatch and handoff expiry. Natural CLI exit transfers only the document path and existing expiry to a detached plain-Node helper; the parent detaches only after an exact IPC ready message confirms argument validation and a referenced expiry timer. Pre-ready error, exit, disconnect, or timeout keeps direct parent expiry cleanup. The helper inherits no loader arguments, environment, handoff, or endpoint token and removes the document at that expiry.

The foundation boots the same complete base-and-Web composition from its declared built binary and direct source entry. An Electron Main caller launches that Runtime child through its own executable with child-only `ELECTRON_RUN_AS_NODE=1`; the Runtime bin consumes the marker before composition so later subprocesses cannot inherit it. Node callers retain their environment, and the setting does not enter Renderer. Terminal, Web, and Desktop presentation work consumes this public API as separate product layers; foundation acceptance alone does not represent those presentation clients or cross-client product acceptance as shipped.
