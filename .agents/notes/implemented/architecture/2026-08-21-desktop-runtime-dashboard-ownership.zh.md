...
干净的 Dashboard 会先执行 cookie 认证的控制 preflight，再执行 `AppWebEntry.run()`。只有两者都成功后，Web root 才设置 `data-harness-dashboard-ready="true"`。Main 会在每次导航中验证精确的干净 URL 并等待该 marker；只有常量 `{"kind":"desktop-dashboard-ready","version":1}` JSONL 输出在每个 Desktop 进程中最多发出一次。在 marker probe 等待期间抢先结算的 abort 或导航失败不能发出 acknowledgement。
