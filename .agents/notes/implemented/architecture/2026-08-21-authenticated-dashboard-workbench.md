...
The historical in-process Web e2e scaffold has no native Runtime client that can mint a handoff. It enables the ordinary Host connection row and supplies a same-origin Dashboard-control shim only to boot the real built `AppWebEntry` and Loader graph. The real handoff and HttpOnly-cookie sequence remains covered by the dedicated Dashboard-ready browser suite.
