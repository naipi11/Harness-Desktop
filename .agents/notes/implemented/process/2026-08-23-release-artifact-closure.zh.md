...
打包 CLI 保留 `harness` 与 `dsh` 两个使用外部 import 的小型 client entry，并把完整 Runtime graph 部署为物理 bundled package dependencies。Payload 保留 `dsh-host-local-runtime` 自己的 `lib/bin.js` 与 `runtime.cordis.yml`、base/Web/headless patch layer、plugin 与 worker entry，以及目标原生依赖，保证按包相对定位的 Runtime spawn 不会反向解析到 CLI entry。Pack owner 只从名称与版本完全相同的 source package 修复缺失的 declared bin，把该 bin 保留在依赖的 files allowlist 中，再重新解压并审计生成的 tarball；standalone 消费过程从不修复外部提供的字节。正式发布调用方在验收前要求 built CLI entry 必须存在；普通未构建测试清单可以跳过，但正式调用缺少该 entry 时会在 CLI tarball 构建前失败。验收过程把 tarball 安装到使用空 cache、offline 模式且禁用 scripts 的新 npm prefix，运行两个由该 prefix 拥有的 help 命令，并先要求已安装的 `harness web --status` 在 home 缺失时返回 typed no-start diagnostic，且不创建 Runtime 文件、browser bootstrap 或后代进程。验收把嵌套的已安装 Runtime bin 作为 stdin-owned child 启动，通过已安装 public connector 观察 `session-opened`，用 `harness` 获取 Web lease，再用 `dsh` 与 `harness` 幂等释放，证明保留的 terminal 仍可使用，然后在结束 Runtime stdin 前关闭两层 attachment；process kill 只作为有界的故障 fallback，graceful join 与 post-kill join 各自具有独立 deadline。物理 Node driver 加载已安装 `lib/bin.js` 引用的 `runCli` export：注入的 installed-app activator 只运行一次且不使用 Runtime 或 browser，default resolver 则在不执行 native launch 或 Electron substitute 的条件下报告当前平台安装路径与脱敏 diagnostic。每个验收子进程只接收显式传入且经过清理的环境，因此 ambient 或来自 checkout 的 Node 与 TypeScript loader hook 始终缺席。Absent-status 进程会有意接收一个来自私有临时根目录、仅由测试拥有的 `NODE_OPTIONS` tripwire preload；该 preload 用于拦截子进程创建，绝不会传给 Runtime、connector helper、lease 命令或 Desktop driver。环境扩展被禁用，包含 checkout 的环境值与 `PATH` entry 始终缺席，npm 通过绝对可执行文件路径运行。Harness 自有源码与测试、凭据、Desktop 产物和过期 hash chunk 都不进入 payload；已发布的第三方依赖仍保留各自 package manifest 选中的内容，包括依赖自有的源码与测试路径。
