...
The old Agent Note already deferred a separate `@harness-desktop/dsh-fs-observation-policy` package. This decision builds that layer and keeps `ctx.fs` close to fsspec-style storage primitives (`info`/`cat`/`open`), without turning it into full fsspec.
...
`@harness-desktop/dsh-fs` shrinks to provider text IO plus guarded text mutation:
