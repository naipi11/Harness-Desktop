...
Bundles named in `dsh.profile.bundles` resolve from the dsh installation first (`@stackstackstack/dsh-base`, `@stackstackstack/dsh-web-app`, `@stackstackstack/dsh-headless`), then from the profile's own `node_modules`, where pnpm installs out-of-tree plugins.
