...
import { entryListSchema } from '@harness-desktop/cordis-plugin-include'
import { evaluate } from '@harness-desktop/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@harness-desktop/dsh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@harness-desktop/dsh-base', '@harness-desktop/dsh-web-app'])
    ...
    for (const name of ['@harness-desktop/dsh-pwsh-sandbox', '@harness-desktop/dsh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@harness-desktop/dsh-base'])
