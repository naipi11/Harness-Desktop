 ...
 * @module @harness-desktop/cli/plugin
 */
...
const DATA_NAMESPACE = productMetadata.dataNamespace
...
function exportsPatch(packageName: string, profileDir: string): boolean {
  let dir: string
  try {
    dir = resolveBundleDir(DATA_NAMESPACE, packageName, INSTALL_ANCHOR, profileDir)
  } catch {
    return false // pnpm reported success yet the package is unresolvable — treat as plain
  }
  const manifest = readProfileManifest(DATA_NAMESPACE, dir)
 ...
 */
function reconcilePlugins(commandName: CliCommandName, before: ProfileManifest, profileDir: string): void {
  const after = readProfileManifest(DATA_NAMESPACE, profileDir)
  ...
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${commandName}: warning: ${packageName} declares no dsh.bundle — installed as a plain dependency, not a profile layer `
...
/**
 * Run one compatible plugin invocation: init if needed, forward to pnpm, reconcile.
 * @param commandName - command name printed in plugin diagnostics.
...
export function runPlugin(commandName: CliCommandName, profile: string, args: readonly string[]): number {
  ...
  const before = readProfileManifest(DATA_NAMESPACE, dir)
  ...
  if (result.error !== undefined) {
    ...
    if (code === 'ENOENT') {
      process.stderr.write(`${commandName}: pnpm not found on PATH — install pnpm to manage profile plugins\n`)
      return 127
    }
    throw result.error
  }
  ...
  if (exitCode === 0) {
    reconcilePlugins(commandName, before, dir)
  } else {
    ...
    process.stderr.write(`${commandName}: pnpm failed in profile directory ${dir}\n`)
    if (args.some(argument => /^git\+|^github:|\.git(?:#|$)/.test(argument))) {
      process.stderr.write(
        `${commandName}: git-hosted plugins build on install via their prepare script, which pnpm blocks until allowed — `
