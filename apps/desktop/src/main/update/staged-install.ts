...
import type { RedactedUpdateArtifact } from '@harness-desktop/dsh-update-policy'

/** Verified bytes and declaration retained only for one in-process update attempt. */
export interface StagedDesktopCandidate {
  /** Redacted signed-manifest selection. */
  readonly artifact: RedactedUpdateArtifact
...
export interface StageAdapter {
  /** @param artifact - selected redacted candidate. @returns verified candidate bytes. */
  download(artifact: RedactedUpdateArtifact): Promise<Uint8Array>
  /**
   * @param bytes - downloaded candidate bytes.
   * @param artifact - declared member set.
   * @returns actual archive members.
   */
  inspect(bytes: Uint8Array, artifact: RedactedUpdateArtifact): Promise<readonly string[]>
  /**
