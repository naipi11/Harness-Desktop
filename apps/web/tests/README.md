...
## Do not import `@harness-desktop/dsh-client-*` here
...
`@harness-desktop/dsh-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@harness-desktop/dsh-client-web` and the boot-manifest
type from `@harness-desktop/dsh-client-modules/client`: booting the real shell is what
...
`@harness-desktop/dsh-client-runtime/client` because `client/runtime` is reachable
