...
The public package family belongs to the `@harness-desktop` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@harness-desktop/node-addon-landlock-run
@harness-desktop/node-addon-landlock-run-<platform>
```
