 ...
 * REAL `@stackstackstack/dsh-sdk-client`, drives one turn over stdio JSON-RPC,
...
} from '@stackstackstack/dsh-acp-snapshot'
import { resolveExampleLaunch } from '@stackstackstack/dsh-loader-smoke'
import { DeepSeekHarness, type HarnessNotification, type RunResult } from '@stackstackstack/dsh-sdk-client'
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@stackstackstack/dsh-system-prompt') return []
