...
A sandbox-consuming executor exposes its configured mode fallback through `ShellExecutor.sandboxMode`. The tool layer asks [`@harness-desktop/dsh-sandbox-policy`](../../packages/sandbox/sandbox-policy/README.md) to resolve each calling session's durable `sandbox/mode` override and immutable cwd into `ShellExecRequest.sandboxPolicy`; a user-approved strictly wider call replaces only the mode. The mode/root/enforcement vocabulary is owned by the [`@harness-desktop/dsh-sandbox` seam](sandbox.md); modes govern file effects only.
...
Source: [`packages/shell/shell-env/src/index.ts:90`](../../packages/shell/shell-env/src/index.ts)
