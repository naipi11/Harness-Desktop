...
| Package | Role |
|---|---|
| `@harness-desktop/dsh-shell` (this) | Service Definition: abstract service + vocabulary types |
| `@harness-desktop/dsh-bash-local` | Service Provider: local subprocesses |
| `@harness-desktop/dsh-bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@harness-desktop/dsh-tool-bash` | the model-facing tool schemas over `ctx.shell` |
