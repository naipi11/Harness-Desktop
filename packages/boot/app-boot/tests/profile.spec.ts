...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@stackstackstack/dsh-base'])
    ...
    expect(readProfileManifest('t', dir).dsh?.profile?.bundles).toEqual(['@stackstackstack/dsh-base'])
...
describe('loadProfile', () => {
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    expect(PROFILE_TEMPLATES.web).toContain('@stackstackstack/dsh-base')
    try {
  ...
  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@stackstackstack/dsh-base': { patch: '[]\n' },
      '@stackstackstack/dsh-web-app': { patch: '[]\n' },
      '@stackstackstack/dsh-headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      '@stackstackstack/dsh-base', '@stackstackstack/dsh-web-app', '@stackstackstack/dsh-headless',
    ])
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).dsh?.profile?.bundles)
      .toEqual(['@stackstackstack/dsh-base', '@stackstackstack/dsh-headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      '@stackstackstack/dsh-base', '@stackstackstack/dsh-web-app', '@stackstackstack/dsh-headless', 'custom-bundle',
    ])
