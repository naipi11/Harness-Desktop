...
| `boot(binName, absoluteConfigPath, patches?, prepare?, bareModuleBaseUrl?)` | 创建根上下文，向 Loader `!!js` 配置表达式暴露已解析的 `harnessHome` 及其 `harnessHomePath(...segments)` helper，并安装 Loader，在配置树条目挂载前执行可选的宿主准备操作（`prepare` 可以使用 Loader，也可以提供由启动器拥有的上下文插槽），再挂载并等待 include 树结算，断言所有条目均已加载并激活，最后返回根上下文——失败时 dispose（资源释放）部分构造的上下文，并以带标签的错误 reject；可选模块基准与 `mountRootInclude` 的解析语义相同 |
...
| `productMetadata` / `ProductMetadata` / `ProductCommandName` | 来自包自有 `product.json` 的稳定 Harness Desktop 标识符，通过 `@harness-desktop/dsh-app-boot/product-metadata` 发布 |
...
本包只拥有稳定的产品标识符。其 `dataNamespace` 有意保持为 `dsh`；运行时默认值属于各自的启动器和消费者，不属于产品元数据。
