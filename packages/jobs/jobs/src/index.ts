 ...
 * process-local registry lives in `@stackstackstack/dsh-jobs-local`.
 * @module @stackstackstack/dsh-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@stackstackstack/dsh-jobs is the abstract job registry seam; load an implementation such as @stackstackstack/dsh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
