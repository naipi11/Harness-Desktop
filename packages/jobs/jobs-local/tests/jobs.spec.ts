...
import { bindScopeParent, createScope, scopeOf } from '@stackstackstack/dsh-scope'
import type { ScopeKey } from '@stackstackstack/dsh-scope'
import { JobId } from '@stackstackstack/dsh-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@stackstackstack/dsh-jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @stackstackstack/dsh-tool-jobs in its composition)')
  })
