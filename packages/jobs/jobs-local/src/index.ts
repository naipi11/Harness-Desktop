 ...
 * @module @harness-desktop/dsh-jobs-local
 */

import { Context } from '@harness-desktop/cordis'
import z from '@harness-desktop/schemastery'
import type { Agent } from '@harness-desktop/dsh-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@harness-desktop/dsh-scope'
import type { ScopeLayer } from '@harness-desktop/dsh-scope'
import { deadline, timeoutOf } from '@harness-desktop/dsh-timeout'
import { JobRegistry, JobId } from '@harness-desktop/dsh-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@harness-desktop/dsh-jobs'
 ...
 * `@harness-desktop/dsh-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @harness-desktop/dsh-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @harness-desktop/dsh-agent)')
    }
