...
import * as SessionStatsPlugin from '@stackstackstack/dsh-session-stats'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@stackstackstack/dsh-session-projection', SessionProjectionRegistry],
    ['@stackstackstack/dsh-session-stats', SessionStatsPlugin],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped session-stats YAML shape and serves whole-log counts', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@stackstackstack/dsh-session-projection'",
      "- name: '@stackstackstack/dsh-session-stats'",
    ])
