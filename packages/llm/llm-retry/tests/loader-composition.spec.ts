...
import LlmRuntime, { createUserMessage, LlmAdapter, LlmError, resolveRetryPolicy  } from '@harness-desktop/dsh-llm'
import type { GenerateOptions, ResolvedRetryPolicy, StreamChunk } from '@harness-desktop/dsh-llm'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@harness-desktop/dsh-llm-retry', retry],
    ['@harness-desktop/dsh-agent-loop', AgentLoop],
  ])
...
describe('real Loader composition', () => {
  ...
  it('loads provider-supplied policy and records recovery through the shipping loop', { timeout: 60_000 }, async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@harness-desktop/dsh-llm-retry'",
      "- name: '@harness-desktop/dsh-agent-loop'",
    ])
