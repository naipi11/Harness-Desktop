...
import Storage from '@stackstackstack/dsh-storage'
import * as StorageDomain from '@stackstackstack/dsh-storage-domain'
import * as StorageJson from '@stackstackstack/dsh-storage-json'
import { remoteMethods } from '@stackstackstack/dsh-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@stackstackstack/dsh-session', SessionStore],
    ['@stackstackstack/dsh-session-persistence-jsonl', JsonlSessionPersistence],
    ['@stackstackstack/dsh-storage', Storage],
    ['@stackstackstack/dsh-storage-json', StorageJson],
    ['@stackstackstack/dsh-storage-domain', StorageDomain],
    ['@stackstackstack/dsh-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      "- name: '@stackstackstack/dsh-session'",
      "- name: '@stackstackstack/dsh-session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@stackstackstack/dsh-storage'",
      "- name: '@stackstackstack/dsh-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@stackstackstack/dsh-storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@stackstackstack/dsh-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
