...
import * as SessionLogDownload from '@harness-desktop/dsh-session-log-export'
...
describe('session-log-download real Loader composition', () => {
  it('discovers and executes /export through the assembled command plane', async () => {
    ...
    await writeFile(configPath, [
      ...
      "- name: '@harness-desktop/dsh-session-log-export'",
      '',
    ].join('\n'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@harness-desktop/dsh-commands', CommandRuntime],
      ['@harness-desktop/dsh-session-log-export', SessionLogDownload],
    ])
