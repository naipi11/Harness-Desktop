# `@harness-desktop/dsh-loader-smoke`
...
`runLoaderSmoke` accepts bin and config paths, optional complete bin arguments, environment overrides, stdin, pre-run setup, and pre-cleanup inspection. It owns the isolated cwd, one Harness home, diagnostics, deadline, termination, EOF, and cleanup. An absolute environment `HARNESS_HOME` override becomes the effective home; relative and tilde-prefixed values fail before startup because callback code cannot share the child's cwd-based resolution. Both callbacks receive the exact absolute value injected into the child. It returns both streams after a zero exit and rejects with both streams on failure.
