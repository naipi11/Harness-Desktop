...
Runtime 组合可用 `ConnectionAuthentication` callback 调用 `mountAuthenticatedConnection()`。该 callback 与既有 trust fence 一起运行于 HTTP bridge、其 Fetch 表示以及两条 WebSocket upgrade，因此可要求已签发的 session cookie 和精确 Runtime Origin，同时不削弱 DNS rebinding 或特权方法策略。普通 `apply()` 仍是未认证部署形式。
