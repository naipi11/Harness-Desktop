...
外壳自给自足（web2 硬性规则）：内核不对任何插件包执行值导入；启动状态 store 与信号在这里手写（`loader-status.ts`），因此即使插件失败，加载页面仍能工作，而此时这一点尤其重要。app-shell 组装（`@harness-desktop/dsh-client-app-shell`，由外壳拥有、背后没有 npm 包的伪配置项）是唯一通过 `registerStatic` 注册的模块；它与任何插件一样，通过 inject 等待 slots、sessions、workspaces、layout 和 locale。
...
完成 settle 的 Dashboard 在普通 root slot 外挂载紧凑且可收起的工作台。文件、差异、终端、产物、任务、空状态和操作文案都跟随共享语言设置。文件面板通过 Runtime 工作台 API 列出已登记工作区中的目录和文件，不依赖选择器能力；失败会明确显示。打开文件使用 Workspace 服务。差异读取所选 Session 的 snapshot，产物读取已完成 Turn 的交付物，任务读取 `todos` 投影。专注模式保留所选 Session 和连接。

终端通过已认证 Runtime 启动按所有者隔离的持久命令 shell，而不是调用 `session.prompt`。它保留目录和变量状态，显示 stdout/stderr，无需模型密钥；关闭终端会停止其运行进程树。这个按行输入的控制台不支持全屏或交互式程序，输出有界且只保存在内存。窄窗口保留对话，工具面板可以收起。
