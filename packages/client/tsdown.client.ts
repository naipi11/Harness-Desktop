 ...
 * Everything else under @harness-desktop/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@harness-desktop\/dsh-(host-apiproxy|session|llm|tools|brand)(\/|$)/
...
const GENERATED_REMOTE = /^@harness-desktop\/dsh-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
 ...
 */
const RUNTIME_STORE_EXEMPTION = '@harness-desktop/dsh-client-runtime/client'
...
function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    plugins: [{
      ...
      // and every other @harness-desktop value import is a build error — a
      ...
      resolveId(source: string) {
        if (!source.startsWith('@harness-desktop/')) return null
