# @harness-desktop/dsh-client-modules
...
The Node half scans live Loader entries for web `dsh.client` packages, resolves each `exports["./client"]`, hashes the built bundle into the boot graph, and serves it with its source map under `/plugins`. Bare entry names resolve package metadata from the config-tree anchor. Built compositions may instead give Loader a physical `file:` entry; the registry finds its nearest `package.json`, uses the manifest `name` as the graph id, and keeps one row while any entry alias for that package qualifies. An unresolvable entry or an anonymous package without a Web declaration remains a non-client row, while malformed client metadata fails on that entry's own incremental pass without poisoning another package's reconciliation. A declaration with `includeWhenDisabled: true` retains only that browser bundle when the configured Host row is disabled because another lifecycle owner mounts its Host half; client connection uses this for Runtime-authenticated routes. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
...
- **No unload bookkeeping of its own** — style removal and fiber teardown ordering live with the HMR driver (`@harness-desktop/dsh-client-hmr`); the loader only inventories owned style tag ids per record.
