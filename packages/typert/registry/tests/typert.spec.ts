...
} from '@harness-desktop/dsh-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@harness-desktop/dsh-typert-protocol'
...
function toolsContribution(schema: z.ZodType = z.object({ name: z.string() })): TypertContribution {
  return {
    package: '@harness-desktop/dsh-tools',
    face: 'host',
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@harness-desktop/dsh-tools', 'ToolInput')).toBe('@harness-desktop/dsh-tools#ToolInput')
    expect(typertPackageKey('@harness-desktop/dsh-tools', 'host')).toBe('@harness-desktop/dsh-tools#host')
    expect(ctx.typert.get('@harness-desktop/dsh-tools#ToolInput')).toMatchObject({
      package: '@harness-desktop/dsh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@harness-desktop/dsh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@harness-desktop/dsh-tools', 'host')).toMatchObject({
      key: '@harness-desktop/dsh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@harness-desktop/dsh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@harness-desktop/dsh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@harness-desktop/dsh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@harness-desktop/dsh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@harness-desktop/dsh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@harness-desktop/dsh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@harness-desktop/dsh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@harness-desktop/dsh-tools#ToolInput')).not.toBe(projected)
  })
