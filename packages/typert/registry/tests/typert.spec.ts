...
} from '@stackstackstack/dsh-typert-registry'
import type {
...
function toolsContribution(schema: z.ZodType = z.object({ name: z.string() })): TypertContribution {
  return {
    package: '@stackstackstack/dsh-tools',
    face: 'host',
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@stackstackstack/dsh-tools', 'ToolInput')).toBe('@stackstackstack/dsh-tools#ToolInput')
    expect(typertPackageKey('@stackstackstack/dsh-tools', 'host')).toBe('@stackstackstack/dsh-tools#host')
    expect(ctx.typert.get('@stackstackstack/dsh-tools#ToolInput')).toMatchObject({
      package: '@stackstackstack/dsh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@stackstackstack/dsh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@stackstackstack/dsh-tools', 'host')).toMatchObject({
      key: '@stackstackstack/dsh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@stackstackstack/dsh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@stackstackstack/dsh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@stackstackstack/dsh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@stackstackstack/dsh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@stackstackstack/dsh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@stackstackstack/dsh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@stackstackstack/dsh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@stackstackstack/dsh-tools#ToolInput')).not.toBe(projected)
  })
