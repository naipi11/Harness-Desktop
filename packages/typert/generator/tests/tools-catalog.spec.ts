...
import type { TypertContribution } from '@harness-desktop/dsh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@harness-desktop/dsh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@harness-desktop/dsh-tools'],
    }).analyze()
    ...
    const artifact = new FaceModelEmitter(host).emit('@harness-desktop/dsh-tools')
    ...
    const record = ctx.typert.getPackage('@harness-desktop/dsh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@harness-desktop/dsh-tools', 'host')).toBeUndefined()
  })
})
