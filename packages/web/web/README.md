...
| Package | Role |
|---|---|
| `@harness-desktop/dsh-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@harness-desktop/dsh-web-search-exa` | Search provider: Exa |
| `@harness-desktop/dsh-web-search-perplexity` | Search provider: Perplexity |
| `@harness-desktop/dsh-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@harness-desktop/dsh-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
