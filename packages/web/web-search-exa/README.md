...
This is an **implementation** package: it registers a provider into `ctx.web`, it does not own the `ctx.web` key and it does not register a model-facing tool (that is `@stackstackstack/dsh-tool-web`). Like `@stackstackstack/dsh-llm-deepseek`, it is a function/namespace plugin (`inject: ['web']`) that registers its backend, not a default-export service.
...
```yaml
- id: web-search-exa
  name: '@stackstackstack/dsh-web-search-exa'
  config:
