...
@stackstackstack/dsh-llm              abstract LLM service + content-block vocabulary
@stackstackstack/dsh-session          event-sourced session log + store
@stackstackstack/dsh-session-title    log-backed title service + deterministic fallback
@stackstackstack/dsh-system-prompt    prompt-section + tool-schema assembly
@stackstackstack/dsh-tools            registry + guarded pre/around/post/final-result pipeline
@stackstackstack/dsh-skill            skill provider registry
@stackstackstack/dsh-skill-filesystem      local filesystem skill provider
@stackstackstack/dsh-agent            agent registry + initiator scope + agent/* events
@stackstackstack/dsh-goal             optional persisted same-session goal domain
@stackstackstack/dsh-tool-goal        optional model-facing goal controls
@stackstackstack/dsh-goal-round-driver     optional same-session goal-round driver
@stackstackstack/dsh-llm-retry        provider-routed request retry policy
@stackstackstack/dsh-jobs-local      generic background-job registry
@stackstackstack/dsh-invariants       configurable invariant registry service
@stackstackstack/dsh-session/invariant
@stackstackstack/dsh-agent/invariant
@stackstackstack/dsh-scope/invariant
@stackstackstack/dsh-agent-loop/invariant
                                  package-owned relational checks
@stackstackstack/dsh-tool-bash        the model-facing bash schema (unless toolBash=false)
@stackstackstack/dsh-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@stackstackstack/dsh-tool-skill       session-prefix skill catalog + model-facing loader schema
@stackstackstack/dsh-tool-jobs       job_output/job_list/job_kill schemas + completion notices
@stackstackstack/dsh-agent-loop       THE concrete loop (gets the forwarded `agents`)
...
```ts
import type { Config } from '@stackstackstack/dsh-agent-spine-demo'
...
例如，`{ invariants: { enabled: true, package_allowlist: ['^@stackstackstack/dsh-'], package_blocklist: ['agent-loop$'] } }` 会让包拥有的配套插件保持挂载，但抑制被阻止的拥有者。Blocklist 匹配优先于 allowlist 匹配；正则表达式与生命周期规则见 [`dsh-invariants`](../../runtime-diagnostics/invariants/README.md)。
