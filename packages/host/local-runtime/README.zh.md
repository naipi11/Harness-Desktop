...
每个 `RuntimeClient`、`TerminalConnection` 与 `DashboardAttachment` 都拥有一个服务端映射的 attachment；经过认证的 parent 无法释放、提交到、取消或控制另一个 parent 的 child id。`close()` 只有在幂等服务端 release 成功后才提交 closed 状态，因此瞬时传输失败可以重试同一次 release，成功后则不会重复发出。关闭 attachment 不会取消活动工作。取消会仅移除本操作尚未 claim 的 inbox message，或在保留 inbox 的同时向已 claim 的精确 Agent 操作发出信号；无关的 queued 与 steering message 会保留。存在关联 turn 时，Runtime 随后等待该 turn，并等待 Agent idle，再只释放该操作的 work lease。

`openTerminal()` 通过已组装的 API owner 创建或恢复，`submit()` 则进入真实 Agent turn。Runtime 将请求的 `rpcId` 与精确 inbox claim、turn 编号、Agent 实例和 `turn/end` 关联，再等待 `agent.whenIdle()` 后才准入 replacement；陈旧 turn 或 cancel completion 无法清除后来的 lease。精确 slash command 通过 `ApiProxy.sessions.prompt` 与已组装 command owner 执行，发出安全的成功文本，记录 `command/run` 与 `command/done`，并因不存在 inbox claim 或 turn 而立即释放其精确 work lease。Terminal event 来自实时 session 和 approval 机制：流式 assistant 文本、tool activity、model／permission 变化及 approval question。只有拥有活动 Agent 操作的 terminal 才能提交 approval response。`runControl()` 会恢复 terminal、查询或选择 model、改变 permission preset、执行已注册 command，或在 `exit` 时只关闭该 terminal；每项操作都核验 owner 并持有 Runtime，unavailable、busy、rejected 或 wrong-owner 请求会拒绝，而不是返回合成成功。
