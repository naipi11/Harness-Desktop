...
规范 Runtime 还以 `system` 信任提供随软件分发的智能体预设列表，其优先级高于用户可写根目录。源码启动读取 CLI 中作为唯一来源的 `config/agent-presets` 目录；Runtime 构建将该目录复制到 `lib/agent-presets` 并声明预设所需的插件依赖，使安装后的客户端无需用户自建预设也能创建首个会话。不会把预设复制到 `HARNESS_HOME`。

`desktop-update` 设置 namespace 通过同一设置提供方存储所选 `stable`、`beta` 或 `nightly` 频道以及一条固定格式的脱敏结果。原生控制只能记录语义版本、固定结果种类和代码之一，以及可选的最后已知良好版本；Dashboard 控制只能读取或变更频道。Runtime 不获取、验证、暂存、应用或回滚产物，也没有生产更新信任根。

只有原生控制可以读取最后一条脱敏结果；Dashboard 控制不能读取或写入该结果。

## 生命周期与租约

使用 cookie 认证的 `POST /_harness/workbench` 只列出已登记工作区根目录及其实际路径后代，并操作按所有者隔离的命令 shell。目录读取有条目和字节上限，shell 输出是内存中的有界尾部。shell 通过现有会清除凭据环境变量的 subprocess 提供方，以当前用户权限运行。这是用户直接操作的命令，不是模型工具或 Agent Turn。释放原生客户端会关闭其 shell，Runtime 关闭时会关闭其余 shell。控制台支持 Windows 上的持久 PowerShell 命令和 POSIX 上的 `/bin/sh` 命令，不支持全屏应用。
...
`RuntimeConnector` 使用调用方的 Node 可执行文件启动匹配的 Runtime 入口。当调用方是 Electron Main 时，只有子进程环境会设置 `ELECTRON_RUN_AS_NODE=1`，使 Electron 可执行文件以 Node 方式运行 Runtime 脚本；普通 Node 调用方保留其现有环境。Runtime bin 会在组合启动前消费该标记，因此后续 agent 子进程与打开的应用不会继承 Electron Node 模式。该设置不会进入 Renderer 或无关的 Electron 子进程。
...
- **更新偏好不是更新器** — Runtime 只持久化频道和脱敏结果；在提供生产信任配置前，受信 manifest 获取、产物验证、安装和回滚均不可用。
