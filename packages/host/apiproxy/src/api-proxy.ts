...
import { installModelSelection } from '@stackstackstack/dsh-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@stackstackstack/dsh-agent'
import type {} from '@stackstackstack/dsh-agent-presets/types'
import { AttachmentError } from '@stackstackstack/dsh-attachment'
import type { ImageAttachmentRef } from '@stackstackstack/dsh-attachment'
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@stackstackstack/dsh-llm'
import { errorChain } from '@stackstackstack/dsh-llm'
import type { ContentBlock, MessageSource } from '@stackstackstack/dsh-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@stackstackstack/dsh-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@stackstackstack/dsh-session'
import type { SessionPersistence } from '@stackstackstack/dsh-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@stackstackstack/dsh-session-query'
import { SubagentError } from '@stackstackstack/dsh-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@stackstackstack/dsh-subagent'
import { isUserInvocable } from '@stackstackstack/dsh-skill'
import type { Workspace, WorkspaceRecord } from '@stackstackstack/dsh-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@stackstackstack/dsh-workspace'
...
} from '@stackstackstack/dsh-agent-presets'
import type { PresetBearingSession } from '@stackstackstack/dsh-agent-presets'
...
import type { SessionRawArtifact } from '@stackstackstack/dsh-session-persistence'
import {
...
import type {} from '@stackstackstack/dsh-jobs'
import type { JobSnapshot } from '@stackstackstack/dsh-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@stackstackstack/dsh-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@stackstackstack/dsh-goal'
import type { GoalRef as CoreGoalRef } from '@stackstackstack/dsh-goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@stackstackstack/dsh-commands'
...
import type {} from '@stackstackstack/dsh-cordis-host-runner/types'
import type {} from '@stackstackstack/dsh-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@stackstackstack/dsh-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@stackstackstack/dsh-settings'
import { credentialRef } from '@stackstackstack/dsh-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@stackstackstack/dsh-session-title'
import type { CallId } from '@stackstackstack/dsh-llm/brand'
import type { ScopeKey } from '@stackstackstack/dsh-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@stackstackstack/dsh-user-approval'
// Side-effect type import: resolves the `approval/request` waterfall and
// `ctx.get('approval')` without a value dependency on the seam (optional composition).
import type {} from '@stackstackstack/dsh-user-approval'
...
} from '@stackstackstack/dsh-user-questions'
import { UserQuestionError } from '@stackstackstack/dsh-user-questions'
import { DirectoryPickerError } from '@stackstackstack/dsh-host-directory-picker'
import {
...
} from '@stackstackstack/dsh-api-remotes'
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @stackstackstack/dsh-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @stackstackstack/dsh-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @stackstackstack/dsh-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @stackstackstack/dsh-credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @stackstackstack/dsh-session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @stackstackstack/dsh-skill', details: {} })
        }
