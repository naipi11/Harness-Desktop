...
`session.prompt` 和 `subagent.prompt` 接受可选的请求本地 `clientTimeZone` 来源信息。若提供该值，Host 会在进入 Agent 前校验 `UTC` 或 IANA Area/Location 并将其规范化；无效输入以 `invalid-time-zone` 拒绝，规范值则与 `rpcId` 一起记录在这条确切的 `user-rpc` 消息上。该值不属于 Session、连接、create、resume 或 fork 状态；非浏览器调用方可以省略它。`session.prompt` 还接受 carrier 取消信号，并把该信号原样传给已识别 slash command 的执行过程。若该信号在 Agent 解析前已中止，或在异步准入或 command 执行期间变为中止状态，该方法会返回 `cancelled`，且不发布 inbox 消息。
...
以 `/` 开头、且只含一个 text block 的精确 `session.prompt` 会先使用与 `command.execute` 相同的 command owner。已识别 command 会在响应的 `command` 槽返回可选的安全成功文本，并且不会准入 user message 或 Agent turn。command 未命中后，普通准入同时要求精确 Agent 的 scope 链可达一个 `dsh-tool-skill` 用户调用消费方，并且该 Agent 的完整 skill 目录中存在允许用户调用的开头名称。目录中有条目但没有该消费方时按完整未命中处理。消费方存在时，目录不完整或读取失败会返回 `internal`；其他完整未命中均返回 `unknown-command`，不会进入模型准入。

## 载体层（`/client` + 根路径）

`AbstractApiClient` 持有全部协议不变量：签发 rpcId、包装／解包信封、Zod 解析、SSE 帧解码、一元请求超时，以及按微任务批处理的信封观测（`subscribeEnvelopes`）；平台子类只提供 `doFetch` 传输环节。`InProcessApiClient` 以 `toFetchHandler(api)` 为基础，仍是同构接点：它运行完整的协议序列化与校验路径而不经过网络，供需要该路径的调用方和载体测试使用。内部 headless fixture 可直连 core；公开 `dsh run <task>` 通过共享 Runtime 产品组装连接。
