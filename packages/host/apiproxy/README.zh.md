...
以 `/` 开头、且只含一个 text block 的精确 `session.prompt` 使用与 `command.execute` 相同的 command owner。已识别 command 会在响应的 `command` 槽返回可选的安全成功文本，并且不会准入 user message 或 Agent turn；未识别的名字返回 `unknown-command`，不会进入模型准入。
