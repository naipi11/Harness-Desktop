...
以 `/` 开头、且只含一个 text block 的精确 `session.prompt` 会先尝试与 `command.execute` 相同的 command owner。已识别 command 会在响应的 `command` 槽返回可选的安全成功文本，并且不会准入 user message 或 Agent turn；未识别的名字继续走普通 Agent 准入，使 skill invocation 保留共享的 slash-input 路径。
