...
以 `/` 开头、且只含一个 text block 的精确 `session.prompt` 会先使用与 `command.execute` 相同的 command owner。已识别 command 会在响应的 `command` 槽返回可选的安全成功文本，并且不会准入 user message 或 Agent turn。若 command 未命中，而开头名称存在于该 Agent 完整 skill 目录且允许用户调用，则请求继续走普通准入，由 `dsh-tool-skill` 在 `agent/pre-step` 注入正文；目录不完整或读取失败时返回 `internal`，完整目录仍未命中时则返回 `unknown-command`，不会进入模型准入。
