...
/**
 * Keyless built-artifact smoke: plain Node imports `@stackstackstack/dsh-lsp` and
 * `@stackstackstack/dsh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@stackstackstack/dsh-lsp')
      const LspLocal = await import('@stackstackstack/dsh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@stackstackstack/dsh-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@stackstackstack/dsh-subprocess-local')
