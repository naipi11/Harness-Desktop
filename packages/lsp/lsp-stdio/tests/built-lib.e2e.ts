...
/**
 * Keyless built-artifact smoke: plain Node imports `@harness-desktop/dsh-lsp` and
 * `@harness-desktop/dsh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      const { Context } = await import('@harness-desktop/cordis')
      const { default: Lsp } = await import('@harness-desktop/dsh-lsp')
      const LspLocal = await import('@harness-desktop/dsh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@harness-desktop/dsh-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@harness-desktop/dsh-subprocess-local')
