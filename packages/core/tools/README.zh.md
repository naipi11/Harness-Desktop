...
- **定义中的 `timeoutMs` 仅作声明之用**：注册表绝不会强制执行截止时间；要强制执行，必须使用 `@harness-desktop/dsh-tool-call-timeout-policy` 包装层。
