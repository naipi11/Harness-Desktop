 ...
 * execution pipeline.
 * @module @stackstackstack/dsh-tools
 */

import { Context, Service } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@stackstackstack/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@stackstackstack/dsh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@stackstackstack/dsh-llm'
import { assertNever, deepFreeze, HarnessError } from '@stackstackstack/dsh-llm'
import type { Agent } from '@stackstackstack/dsh-agent'
import { snapshotJsonValue } from '@stackstackstack/dsh-session'
import type { JsonValue, UserMessage } from '@stackstackstack/dsh-session'
import type { ToolProviderResult } from '@stackstackstack/dsh-system-prompt'
import type { CodeRuntime } from '@stackstackstack/dsh-code-runtime'
...
export type { JsonValue } from '@stackstackstack/dsh-session'
...
// lives in its own UI-facing module; re-export it so `@stackstackstack/dsh-tools`
...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@stackstackstack/dsh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@stackstackstack/dsh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@stackstackstack/dsh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@stackstackstack/dsh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@stackstackstack/dsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @stackstackstack/dsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
