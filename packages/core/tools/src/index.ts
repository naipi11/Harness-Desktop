 ...
 * execution pipeline.
 * @module @harness-desktop/dsh-tools
 */

import { Context, Service } from '@harness-desktop/cordis'
import z from '@harness-desktop/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@harness-desktop/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@harness-desktop/dsh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@harness-desktop/dsh-llm'
import { assertNever, deepFreeze, HarnessError } from '@harness-desktop/dsh-llm'
import type { Agent } from '@harness-desktop/dsh-agent'
import { snapshotJsonValue } from '@harness-desktop/dsh-session'
import type { JsonValue, UserMessage } from '@harness-desktop/dsh-session'
import type { ToolProviderResult } from '@harness-desktop/dsh-system-prompt'
import type { CodeRuntime } from '@harness-desktop/dsh-code-runtime'
...
export type { JsonValue } from '@harness-desktop/dsh-session'
...
// lives in its own UI-facing module; re-export it so `@harness-desktop/dsh-tools`
...
declare module '@harness-desktop/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@harness-desktop/dsh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@harness-desktop/dsh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@harness-desktop/dsh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@harness-desktop/dsh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@harness-desktop/dsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @harness-desktop/dsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
