...
import type { Scope } from '@harness-desktop/dsh-scope'
...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@harness-desktop/dsh-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@harness-desktop/dsh-tools'
import type { Agent } from '@harness-desktop/dsh-agent'
import { Session, SessionId } from '@harness-desktop/dsh-session'
import type { JsonValue, SessionEventMap } from '@harness-desktop/dsh-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@harness-desktop/dsh-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
