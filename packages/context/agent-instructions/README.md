...
Instructions from: $HARNESS_HOME/AGENTS.md
...
The user-global file is always `$HARNESS_HOME/AGENTS.md` with no local overlay; `harnessHome` is injected by the host, and both candidate lists only control project scopes. A non-positive or non-finite render budget disables both baseline and dynamic loading; configured `maxSourceBytes` must be a positive integer.
...
- **Candidate semantics stay intentionally small** — lowercase names, `.claude/rules/`, and `@path` imports are not interpreted; project scopes load `AGENTS.local.md`/`CLAUDE.local.md` overlays by default, but the user-global `$HARNESS_HOME` scope has no local overlay and other custom names require explicit candidate configuration.
