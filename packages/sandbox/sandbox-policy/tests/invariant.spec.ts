...
import SessionStore, { type Session, type SessionEvent } from '@harness-desktop/dsh-session'
import InvariantRegistry, { InvariantError } from '@harness-desktop/dsh-invariants'
import * as SandboxPolicyInvariant from '@harness-desktop/dsh-sandbox-policy/invariant'
...
describe('sandbox-policy invariants', () => {
  ...
  it('rejects and attributes an unknown durable sandbox mode', async () => {
    ...
    expect(() => { ctx.emit('session/event', {} as Session, modeEvent('host-root')) })
      .toThrow(new InvariantError('@harness-desktop/dsh-sandbox-policy', 'sandbox/mode carries unknown mode "host-root"'))
  })
  ...
  it('rejects an unknown mode already present on late registration', async () => {
    ...
    await expect(ctx.plugin(SandboxPolicyInvariant).then(() => undefined)).rejects.toMatchObject({
      code: 'INVARIANT',
      packageName: '@harness-desktop/dsh-sandbox-policy',
    })
  })
})
