 ...
 * @module @harness-desktop/dsh-sandbox-local
 */
...
} from '@harness-desktop/node-addon-landlock-run'
import { Context } from '@harness-desktop/cordis'
import z from '@harness-desktop/schemastery'
import { assertNever } from '@harness-desktop/dsh-llm'
import { SandboxProvider, SandboxUnavailableError } from '@harness-desktop/dsh-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@harness-desktop/dsh-sandbox'
import type { SessionId } from '@harness-desktop/dsh-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@harness-desktop/dsh-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@harness-desktop/dsh-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@harness-desktop/dsh-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@harness-desktop/dsh-sandbox-windows-acl/src/runner.ts'))
