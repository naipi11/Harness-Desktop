...
const WORKSPACE_ROOT_PACKAGE = '@stackstackstack/dsh-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@stackstackstack/')) throw new Error(`${normalized} must name an @stackstackstack package`)
...
class DshFamily extends ReleaseFamily {
  ...
  readonly installedEntry = { packageName: '@stackstackstack/dsh', binPath: 'lib/bin.js' }
}
