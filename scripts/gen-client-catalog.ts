...
/** Drop the `@harness-desktop/dsh-` prefix so rows stay readable. */
function shortPackage(name: string): string {
  return name.replace('@harness-desktop/dsh-', '')
}
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' *',
    ' * @module @harness-desktop/dsh-cordis-client-runner/client/slot-catalog',
    ' */',
    '',
    '/* jscpd:ignore-start */',
