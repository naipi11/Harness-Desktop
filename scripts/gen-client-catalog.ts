...
/** Drop the `@stackstackstack/dsh-` prefix so rows stay readable. */
function shortPackage(name: string): string {
  return name.replace('@stackstackstack/dsh-', '')
}
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' *',
    ' * @module @stackstackstack/dsh-cordis-client-runner/client/slot-catalog',
    ' */',
    '',
    '/* jscpd:ignore-start */',
