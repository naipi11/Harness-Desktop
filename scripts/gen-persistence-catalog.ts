...
const SESSION_PACKAGE = '@stackstackstack/dsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@stackstackstack/dsh-session/types'
 ...
 * top-level declaration (in `@stackstackstack/dsh-session`) and any declaration
 * merge inside a `declare module '@stackstackstack/dsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @stackstackstack/dsh-session/known-event-types',
    ' */',
    '',
    '/**',
