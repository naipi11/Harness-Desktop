...
const SESSION_PACKAGE = '@harness-desktop/dsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@harness-desktop/dsh-session/types'
 ...
 * top-level declaration (in `@harness-desktop/dsh-session`) and any declaration
 * merge inside a `declare module '@harness-desktop/dsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @harness-desktop/dsh-session/known-event-types',
    ' */',
    '',
    '/**',
