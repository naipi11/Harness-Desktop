...
const MERGE_HEAD = /declare module ['"](?:@harness-desktop\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@harness-desktop/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@harness-desktop/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@harness-desktop/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@harness-desktop/cordis'` Context merge
...
/**
 * Every event name a `declare module '@harness-desktop/cordis'` Events merge declares in one
