...
const RELEASE_ENTRY_PACKAGE = '@harness-desktop/cli'
...
class WorkspacePackageSet {
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@deepseek-ai/') && !name.startsWith('@harness-desktop/')) {
        throw new Error(`${manifestPath} must name an @deepseek-ai or @harness-desktop package`)
      }
...
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@harness-desktop/cli/lib/bin.js')
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!(name.startsWith('@deepseek-ai/') || name.startsWith('@harness-desktop/')) || name === '@deepseek-ai/dsh-root')) {
