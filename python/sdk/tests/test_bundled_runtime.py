...
- id: session-checkpoints
  name: '@stackstackstack/dsh-session-checkpoint-policy'
- id: subprocess
  name: '@stackstackstack/dsh-subprocess-local'
- id: bash
  name: '@stackstackstack/dsh-bash-local'
  config:
    cwd: '.'
- id: todo
...
def test_bundled_runtime_surfaces_unbundled_plugin_failure(tmp_path: Path, mode: str) -> None:
    ...
    (tmp_path / "cordis.yml").write_text(
        "- id: missing\n  name: '@stackstackstack/dsh-does-not-exist'\n"
    )
    ...
    assert "@stackstackstack/dsh-does-not-exist" in str(excinfo.value)
